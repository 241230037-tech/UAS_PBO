    ...
    def execute_query(self, sql, params=(), fetch=None):
        ...
        try:
            ...
            if conn:
                conn.rollback()
            raise DatabaseError(
                f"Terjadi kesalahan database saat eksekusi: {e}")
        finally:
            if conn:
                conn.close()
...
class User:
    ...
    def __init__(self, username, password, nama, role, id_user=None):
        ...
        # Stored hashed or raw depending on source, validation handles it
...
class Transaksi:
    ...
    def kembalian(self, val):
        ...
        if val_f < 0:
            raise ValueError(
                "Nominal kembalian kurang dari 0! Uang bayar tidak mencukupi.")
...
class UserManager(ModelManager):
    ...
    def get_all(self):
        ...
        rows = self.db.execute_query(
            "SELECT * FROM User ORDER BY id_user DESC", fetch="all")
        users = []
        for r in rows:
            users.append(User(
                id_user=r["id_user"],
    ...
    def update(self, user_obj: User, new_password=None):
        ...
        if new_password and new_password.strip():
            ...
            self.db.execute_query(
                ...
                (user_obj.username, hashed, user_obj.nama,
                 user_obj.role, user_obj.id_user)
            )
        else:
            self.db.execute_query(
...
class BarangManager(ModelManager):
    ...
    def generate_next_id(self):
        ...
        row = self.db.execute_query(
            "SELECT id_barang FROM Barang ORDER BY id_barang DESC LIMIT 1", fetch="one")
        if not row:
            return "BRG-0001"
    ...
    def create(self, barang_obj: Barang):
        ...
        self.db.execute_query(
            ...
            (barang_obj.id_barang, barang_obj.nama_barang,
             barang_obj.kategori, barang_obj.harga, barang_obj.stok)
        )

    def get_all(self):
        """Fetches all inventory items."""
        rows = self.db.execute_query(
            "SELECT * FROM Barang ORDER BY id_barang ASC", fetch="all")
        items = []
        for r in rows:
            items.append(Barang(
    ...
    def get_by_id(self, id_barang):
        ...
        row = self.db.execute_query(
            "SELECT * FROM Barang WHERE id_barang = ?", (id_barang,), fetch="one")
        if row:
            return Barang(
    ...
    def update(self, barang_obj: Barang):
        ...
        self.db.execute_query(
            ...
            (barang_obj.nama_barang, barang_obj.kategori,
             barang_obj.harga, barang_obj.stok, barang_obj.id_barang)
        )
...
class TransaksiManager(ModelManager):
    ...
    def create(self, tx_obj: Transaksi):
        ...
        try:
            ...
            for item in tx_obj.details:
                cursor.execute(
                    "SELECT stok, nama_barang FROM Barang WHERE id_barang = ?", (item.id_barang,))
                res = cursor.fetchone()
                if not res:
                    raise ValueError(
                        f"Barang {item.id_barang} tidak ditemukan di database!")

                curr_stock = res["stok"]
                nama_b = res["nama_barang"]
                if curr_stock < item.jumlah:
                    raise ValueError(
                        f"Stok untuk '{nama_b}' tidak mencukupi! (Stok: {curr_stock}, Beli: {item.jumlah})")

            # 1. Insert Transaksi header
            cursor.execute(
                "INSERT INTO Transaksi (id_transaksi, tanggal, pelanggan, kasir, total, bayar, kembalian) VALUES (?, ?, ?, ?, ?, ?, ?)",
                (tx_obj.id_transaksi, tx_obj.tanggal, tx_obj.pelanggan,
                 tx_obj.kasir, tx_obj.total, tx_obj.bayar, tx_obj.kembalian)
            )
    ...
    def get_all(self):
        ...
        rows = self.db.execute_query(
            "SELECT * FROM Transaksi ORDER BY tanggal DESC", fetch="all")
        txs = []
        for r in rows:
    ...
    def get_total_count(self):
        ...
        row = self.db.execute_query(
            "SELECT COUNT(*) FROM Transaksi", fetch="one")
        return row[0] if row else 0
